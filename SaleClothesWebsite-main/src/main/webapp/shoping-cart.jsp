								...
								<c:if test="${customerCart != null}">
									...
									<tr class="table_row">
										<td class="column-0" style="">
											<input class="ui-checkbox" type = checkbox name="checkedProduct" value="<%=product.getProductId() + " " + product.getSizeId() + " " + product.getColorId()%>">
										</td>

										<td class="column-1">
											<a href="cart?action=add&id=<%=product.getProductId()%>&quantity=-<%=product.getQuantity()%>&size=<%=product.getSizeId()%>&color=<%=product.getColorId()%>">
										...
										<td class="column-2"><%= product.assignName()%>
											(Size <%=entityManager.find(entity.SizeEntity.class, product.getSizeId()).getSizeName()%> / Color
											<%=entityManager.find(ColorEntity.class, product.getColorId()).getColorName()%>)
											<%--Size <%=product.getSizeId()%> / Color <%=product.getColorId()%>--%>
										</td>
										...
										<td class="column-4" style="align-content: center">
											<div class="wrap-num-product flex-w m-l-auto m-r-0">
												<a class="btn-num-product-down cl8 hov-btn3 trans-04 flex-c-m"
												   href="cart?action=add&id=<%=product.getProductId()%>&size=<%=product.getSizeId()%>&color=<%=product.getColorId()%>&quantity=-1">
